...
`.fit()` | `.predict()` - не прокатит, ибо иногда надо хорошо поресерчить.)
...
🔊 **Запись:** [▶️ `2026-06-07 20.33.07.wav`](2026-06-07%2020.33.07.wav) — нажмите, чтобы прослушать.

<audio controls preload="none" src="2026-06-07%2020.33.07.wav">
  Ваша площадка не поддерживает встроенный плеер —
  <a href="2026-06-07%2020.33.07.wav">скачайте запись здесь</a>.
</audio>

> ℹ️ Встроенный плеер `<audio>` работает на Hugging Face, GitLab, в VS Code и при локальном
> просмотре, но **GitHub вырезает HTML-аудио** из README. На GitHub пользуйтесь ссылкой выше
> (либо мы прикрепим запись как ассет релиза — тогда GitHub покажет плеер).

![Стриминг LogReg: вероятность во времени](img/09_stream_test.png)

Первый раз для триггер-слова вероятность немного недотянула до порога. Потом было 2 верных срабатывания (вероятность пересекла порог).
Красная кривая — как раз вероятность «Акылай», которую модель выдаёт для каждого положения окна.
...
$$
\Delta c(m,i)  =  \frac{\sum_{\theta=1}^{\Theta} \theta [c(m+\theta,i) - c(m-\theta,i)]}
...
$$
\text{bandwidth}(m)  = 
\left( \frac{\sum_{k} |X(m,k)| (f_k - \text{centroid}(m))^{2}}
...
$$
f_R(m) = \min\{ f_K  :  \sum_{k: f_k \le f_K} |X(m,k)|  \ge  \rho \sum_{k} |X(m,k)| \}.
$$
...
$$
\text{ZCR}(m)  =  \frac{1}{2L}\sum_{n}|\mathrm{sgn} x(n) - \mathrm{sgn} x(n-1)|.
$$
...
\sigma_\phi = \sqrt{\frac{1}{T}\sum_{m=1}^{T}(\phi(m) - \mu_\phi)^{2}} .
$$
